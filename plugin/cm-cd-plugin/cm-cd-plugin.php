...
function cm_cd_title_table_after_content( $content ) {
	...
	if (is_singular('cd_album') && !empty($multiArray)) {
		...
		for($multiArray->rewind(); $multiArray->valid(); $multiArray->next()) {
			$cdEntry = $multiArray->current();
			$num = $multiArray->key();
			$number = $num +1;
			$content .= '<tr><td>'.$cdEntry->getTitle().'</td>';
			$link = $cdEntry->getLink();
			if(cm_cd_check_url_for_mp3($link)) {
				$content.= '<td>'.cm_cd_mp3_to_player($link).'</td></tr>';
			}
			else {
				$content.= '<td></td></tr>';
			}
...
function cm_cd_metabox_content($cd) {
	...
	global $separator;
	// catch multi-metadata as an ArrayList
	...
	echo'<p>here</p>';
	echo'<p>'.$multiArray.'</p>';
	//echo '<p>'.$multiArray.'</p>'; // runtime error
	// cycle through the array (value contains the multi-entry)
	if($multiArray instanceof ArrayList) {
		echo '<p>ArrayList</p>';
		for($multiArray->rewind(); $multiArray->valid(); $multiArray->next()) {
			$cdEntry = $multiArray->current();
			$num = $multiArray->key();
			$number = $num +1;
			$factory->loadCdEntry($number, $cdEntry);
			$factory->loadIds('haken'.$number, 'name'.$number, 'link'.$number);
			$factory->loadJS('button'.$number, 'url'.$number);
			echo $factory->createdEditObject()->createCdEntry();
		}
	}
...
function cm_cd_save_meta($cd_id) {
	...
	// read old meta data [multi] (ArrayList of CdEntries
	...
	if($oldArray instanceof ArrayList) {
		for($oldArray->rewind(); $oldArray->valid(); $oldArray->next()) {
			$cdEntry = $oldArray->current();
			$num = $oldArray->key();
			$number = $num +1;
			$title = strip_tags($_POST[('name'.$number)]);
			$link = strip_tags($_POST[('link'.$number)]);
			$entry = new CdEntry($title, $link);
			if( isset($_POST[('haken'.$number)]) ) {
				$newArray->add($entry);
			}
		}
	}
	...
	if( (isset($_POST['neu'])) || (isset($_POST['neulink'])) ) {
		...
		if( $newInput != '') {
			$entry = new CdEntry($newInput, $newLink);
			$newArray->add($entry);
		}
	}
	
	update_post_meta($cd_id, 'multi', $newArray);
}
